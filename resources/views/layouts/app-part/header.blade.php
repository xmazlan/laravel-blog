<header class="flex items-center justify-between border-b border-gray-100 px-6 py-3">
    <div class="flex items-center" id="header-left">
        <x-application-logo />
        <div class="top-menu ml-10">
            <ul class="flex space-x-4">
                <li>
                    <a class="flex items-center space-x-2 text-sm text-blue-500 hover:text-blue-900"
                        href="{{ route('home.welcome') }}">
                        Home
                    </a>
                </li>

                <li>
                    <a class="flex items-center space-x-2 text-sm text-gray-500 hover:text-blue-500"
                        href="{{ route('posts.index') }}">
                        Blog
                    </a>
                </li>

                <li>
                    <a class="flex items-center space-x-2 text-sm text-gray-500 hover:text-blue-500"
                        href="http://laravel-blog.test/blog">
                        About Us
                    </a>
                </li>

                <li>
                    <a class="flex items-center space-x-2 text-sm text-gray-500 hover:text-blue-500"
                        href="http://laravel-blog.test/blog">
                        Contact Us
                    </a>
                </li>

                <li>
                    <a class="flex items-center space-x-2 text-sm text-gray-500 hover:text-blue-500"
                        href="http://laravel-blog.test/blog">
                        Terms
                    </a>
                </li>

            </ul>
        </div>
    </div>
    <div class="flex items-center md:space-x-6" id="header-right">
        <div class="flex space-x-5">
            @auth()
                <div class="relative ms-3">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                <button
                                    class="flex rounded-full border-2 border-transparent text-sm transition focus:border-gray-300 focus:outline-none">
                                    <img class="h-8 w-8 rounded-full object-cover"
                                        src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}" />
                                </button>
                            @else
                                <span class="inline-flex rounded-md">
                                    <button
                                        class="inline-flex items-center rounded-md border border-transparent bg-white px-3 py-2 text-sm font-medium leading-4 text-gray-500 transition duration-150 ease-in-out hover:text-gray-700 focus:bg-gray-50 focus:outline-none active:bg-gray-50"
                                        type="button">
                                        {{ Auth::user()->name }}

                                        <svg class="-me-0.5 ms-2 h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none"
                                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                                        </svg>
                                    </button>
                                </span>
                            @endif
                        </x-slot>

                        <x-slot name="content">
                            <!-- Account Management -->
                            <div class="block px-4 py-2 text-xs text-gray-400">
                                {{ __('Manage Account') }}
                            </div>

                            <x-dropdown-link href="{{ route('profile.show') }}">
                                {{ __('Profile') }}
                            </x-dropdown-link>

                            @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                                <x-dropdown-link href="{{ route('api-tokens.index') }}">
                                    {{ __('API Tokens') }}
                                </x-dropdown-link>
                            @endif

                            <div class="border-t border-gray-200"></div>

                            <!-- Authentication -->
                            <form method="POST" action="{{ route('logout') }}" x-data>
                                @csrf

                                <x-dropdown-link href="{{ route('logout') }}" @click.prevent="$root.submit();">
                                    {{ __('Log Out') }}
                                </x-dropdown-link>
                            </form>
                        </x-slot>
                    </x-dropdown>
                </div>
            @else
                <a class="flex items-center space-x-2 text-sm text-gray-500 hover:text-blue-500"
                    href="http://laravel-blog.test/login">
                    Login
                </a>
                <a class="flex items-center space-x-2 text-sm text-gray-500 hover:text-blue-500"
                    href="http://laravel-blog.test/register">
                    Register
                </a>
            @endauth
        </div>
    </div>
</header>
